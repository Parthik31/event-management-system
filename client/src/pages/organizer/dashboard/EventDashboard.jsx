...
const EventDashboard = () => {
  ...
  const { data, loading, error, lastUpdated, forceRefresh } = useLiveAnalytics('/finance/organizer/event/dashboard', 30000);
  ...
  const topEvents = Array.isArray(data?.topEvents) && data.topEvents.length
    ? data.topEvents
    : reports
        .filter((event) => Number(event.grossRevenue || 0) > 0)
        .sort((left, right) => Number(right.grossRevenue || 0) - Number(left.grossRevenue || 0))
        .slice(0, 6)
        .map((event) => ({ name: event.title, revenue: event.grossRevenue }));
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-6">
        <section className="rounded-[28px] border border-orange-100 bg-white p-6 shadow-[0_20px_60px_rgba(249,115,22,0.08)]">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div className="space-y-3">
              ...
              <div>
                <h1 className="text-3xl font-black tracking-tight text-slate-900 md:text-4xl">Event dashboard</h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600 md:text-base">
                  Monitor live ticket revenue, approved event activity, and attendee movement from one premium operations workspace.
                </p>
              </div>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <MetricCard
            ...
            subtitle="Live gross collections across your events"
            accent="from-orange-500 to-orange-600"
            icon={<Wallet className="h-5 w-5" />}
          />
          <MetricCard
            title="Active Events"
            value={formatNumber(kpis.activeEvents)}
            subtitle="Approved and currently live listings"
            icon={<LayoutGrid className="h-5 w-5" />}
          />
          <MetricCard
            title="Tickets Sold"
            value={formatNumber(kpis.ticketsSold)}
            subtitle="Confirmed tickets sold in real time"
            icon={<Ticket className="h-5 w-5" />}
          />
          <MetricCard
            title="Total Users"
            value={formatNumber(kpis.totalUsers)}
            subtitle="Unique customers who booked your events"
        ...
        <section className="rounded-[28px] border border-orange-100 bg-white p-6 shadow-[0_16px_48px_rgba(15,23,42,0.05)]">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">Manage approved, pending, and rejected events without leaving your dashboard.</p>
            </div>
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {filteredEvents.length ? (
                  filteredEvents.map((event) => (
                    <tr key={event.id} className="transition hover:bg-orange-50/40">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="mt-1 text-sm text-slate-500">{event.location || 'Venue to be announced'}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-bold ${statusStyles[event.status] || statusStyles.Pending}`}>
                          {event.status || 'Pending'}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-sm text-slate-600">
                        <div className="font-medium text-slate-800">{event.date || 'TBA'}</div>
                        <div className="mt-1 text-slate-500">{event.time || 'Time pending'}</div>
                      </td>
                      <td className="px-6 py-4 text-right text-sm font-semibold text-slate-700">{formatNumber(event.ticketsSold)}</td>
                      <td className="px-6 py-4 text-right text-sm font-semibold text-slate-900">{formatCurrency(event.grossRevenue)}</td>
                      <td className="px-6 py-4 text-right text-sm font-semibold text-slate-700">{formatNumber(event.totalUsers)}</td>
                      <td className="px-6 py-4 text-right">
                        <button
                          type="button"
                          onClick={() => navigate(`/organizer/events/edit/${event.id}`)}
                          className="inline-flex items-center gap-2 rounded-2xl border border-orange-200 bg-orange-50 px-4 py-2 text-sm font-semibold text-orange-700 transition hover:bg-orange-100"
                        >
                  ...
                  <tr>
                    <td colSpan="7" className="px-6 py-20 text-center">
                      <div className="mx-auto max-w-md">
                        ...
                        <p className="mt-2 text-sm text-slate-500">Try a different search term or adjust the status filter.</p>
                      </div>
                    </td>
                  </tr>
                )}
          ...
          <div className="mt-6 grid gap-4 lg:hidden">
            {filteredEvents.length ? (
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="line-clamp-1 font-semibold text-slate-900">{event.title}</p>
                          <p className="mt-1 text-sm text-slate-500">{event.location || 'Venue to be announced'}</p>
                        </div>
                      ...
                      <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                        <MobileStat label="Date" value={event.date || 'TBA'} />
                        <MobileStat label="Tickets" value={formatNumber(event.ticketsSold)} />
                        <MobileStat label="Revenue" value={formatCurrency(event.grossRevenue)} />
                        <MobileStat label="Users" value={formatNumber(event.totalUsers)} />
                      </div>
                      <button
                        type="button"
                        onClick={() => navigate(`/organizer/events/edit/${event.id}`)}
                        className="mt-4 inline-flex items-center gap-2 rounded-2xl border border-orange-200 bg-orange-50 px-4 py-2 text-sm font-semibold text-orange-700"
                      >
                        <Edit3 className="h-4 w-4" />
                        Edit Event
                      </button>
              ...
              <div className="rounded-3xl border border-dashed border-orange-200 bg-orange-50/50 px-6 py-12 text-center">
                <p className="text-lg font-semibold text-slate-900">No matching events</p>
                <p className="mt-2 text-sm text-slate-500">Try a different filter or create a fresh event listing.</p>
              </div>
            )}
          </div>
